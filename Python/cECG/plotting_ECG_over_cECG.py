# -*- coding: utf-8 -*-
"""
Created on Fri Dec  8 15:37:28 2017
...
if ux:
       folder1=('/home/310122653/Pyhton_Folder/cECG/Matrices/')
else:
       folder1=('C:/Users/310122653/Dropbox/PHD/python/cECG/Matrices/')
if ux:  
       folder2=('/home/310122653/Pyhton_Folder/cECG/cMatrices/')
else:
       folder2=('C:/Users/310122653/Dropbox/PHD/python/cECG/cMatrices/')
...
for j in range(len(dateien_each_patient)): # j=0 Features  j=1 Annotations
    for k in range(len(Neonate)):
        Dateipfad=folder1+dateien_each_patient[j]+Neonate[k]+"_win_"+windowlength+".mat" #Building foldername
        sio.absolute_import   
...
FeatureMatrix_each_patient_all_ECG=[val[:,lst] for sb, val in enumerate(FeatureMatrix_each_patient_all)] # selecting only the features in lstv               
# IMPORTING *.MAT FILES
for j in range(len(dateien_each_patient)): # j=0 Features  j=1 Annotations
    for k in range(len(Neonate)):
        Dateipfad=folder1+dateien_each_patient[j]+Neonate[k]+"_win_"+windowlength+".mat" #Building foldername
...
#            AnnotMatrix_each_patient[k]= np.delete(AnnotMatrix_each_patient[k],(1,2), axis=1) #Reduce AnnotationMatrix to Nx1
#            AnnotMatrix_each_patient[k]=AnnotMatrix_each_patient[k][~np.isnan(AnnotMatrix_each_patient[k]).any(axis=1)]#deleting NAN and turning Matrix to datapoints,Features
FeatureMatrix_each_patient_all_cECG=[val[:,lst] for sb, val in enumerate(FeatureMatrix_each_patient_all)] # selecting only the features in lstv               
...
for i in range(len(FeatureMatrix_each_patient_all)):
       if scaling=='Z': 
              ...
              FeatureMatrix_each_patient_all_ECG[i]=sc.transform(FeatureMatrix_each_patient_all_ECG[i])
              FeatureMatrix_each_patient_all_cECG[i]=sc.transform(FeatureMatrix_each_patient_all_cECG[i])
...
if ux:
       Sessionfolder.append('/home/310122653/Pyhton_Folder/cECG/Matrices/Sessions/')
else:
       Sessionfolder.append('C:/Users/310122653/Dropbox/PHD/python/cECG/Matrices/Sessions/')
if ux:
       Sessionfolder.append('/home/310122653/Pyhton_Folder/cECG/cMatrices/Sessions/')
else:
       Sessionfolder.append('C:/Users/310122653/Dropbox/PHD/python/cECG/cMatrices/Sessions/')
...
for n in [0,1]:
       ...
       for K in range(len(Neonate)):      
       #       SessionFileList=[]
              Dateien=glob.glob(Sessionfolder[n] +'FeatureMatrix_'+Neonate[K]+ '_**')
       ...
       if n==0:       
              FeatureMatrix_each_patient_fromSession_ECG=[val[:,lst] for sb, val in enumerate(FeatureMatrix_each_patient_fromSession)] # selecting only the features used iin lst
       elif n==1:
              FeatureMatrix_each_patient_fromSession_cECG=[val[:,lst] for sb, val in enumerate(FeatureMatrix_each_patient_fromSession)] # selecting only the features used iin lst
